...
class Strategy:
    ...
    def __getSize(self, openPrice, stopLossForOpenOrder):
        return self.__transactions.at[len(self.__transactions) - 1, 'balance'] * self.__kellyF / abs(
            openPrice - stopLossForOpenOrder)
    ...
    def __generateTransactions(self):
        ...
        for i in range(self.__periodOfLossLimit - 1, len(self.__currencyData)):
            ...
            if (self.__currencyData.at[i, 'index'] == 1) and (i < (len(self.__currencyData) - 1)):
                ...
                self.__openOrder(openDate, openTime, 'buy', self.__getSize(openPrice, stopLossForOpenOrder), openPrice,
                                 stopLossForOpenOrder, takeProfit)
                ...
                self.__openOrder(openDate, openTime, 'sell', self.__getSize(openPrice, stopLossForOpenOrder), openPrice,
                                 stopLossForOpenOrder, takeProfit)
