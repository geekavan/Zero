...
class Strategy:
    ...
    def setStrategyAugment(self, R, fastEMA, slowEMA, periodOfLossLimit, lossLimit):
        self.__R = R
    ...
    def __caculateIndex(self):
        ...
        for i in range(1, len(self.__currencyData)):
            ...
            self.__currencyData.at[i, 'macd'] = self.__currencyData.at[i, 'fastEMA'] - self.__currencyData.at[
                i, 'slowEMA']
            if (i >= 3) and (self.__currencyData.at[i - 3, 'macd'] > 0) and (self.__currencyData.at[i - 2, 'macd'] < 0) and (self.__currencyData.at[i - 1, 'macd'] < 0) and (self.__currencyData.at[i, 'macd'] < 0):
                self.__currencyData.at[i, 'index'] = -1
            elif (i >= 3) and (self.__currencyData.at[i - 3, 'macd'] < 0) and (self.__currencyData.at[i - 2, 'macd'] > 0) and(self.__currencyData.at[i - 1, 'macd'] > 0) and (self.__currencyData.at[i, 'macd'] > 0):
                self.__currencyData.at[i, 'index'] = 1
            else:
    ...
    def __generateTransactions(self):
        ...
        for i in range(self.__periodOfLossLimit - 1, len(self.__currencyData)):
            ...
            if (self.__currencyData.at[i, 'index'] == 1) and (i < (len(self.__currencyData) - 1)):
                ...
                if (openPrice - priceMin) > self.__lossLimit:
                    stopLossForOpenOrder = openPrice - self.__lossLimit
                elif (openPrice - priceMin) <= self.__lossLimit:
                    stopLossForOpenOrder = priceMin
                takeProfit = self.__currencyData.at[i + 1, 'open'] + self.__R * (
                        self.__currencyData.at[i + 1, 'open'] - priceMin)
                self.__openOrder(openDate, openTime, 'buy', self.__getSize(), openPrice, stopLossForOpenOrder, takeProfit)
                ...
                if (priceMax - openPrice) > self.__lossLimit:
                    stopLossForOpenOrder = openPrice + self.__lossLimit
                elif (priceMax - openPrice) <= self.__lossLimit:
                    stopLossForOpenOrder = priceMax
                takeProfit = self.__currencyData.at[i + 1, 'open'] + self.__R * (
                        self.__currencyData.at[i + 1, 'open'] - priceMax)
                self.__openOrder(openDate, openTime, 'sell', self.__getSize(), openPrice, stopLossForOpenOrder, takeProfit)
